...
def _terminal_key(value):
    ...
    if not base:
        raise ValueError("invalid terminal '{}'".format(value))
    return base
    ...
    for bus, entries in connections.items():
        if bus.startswith("RBUS"):
            for terminal in entries:
                terminal_key = _terminal_key(terminal)
                sw_pin = pin_to_sw_matrix[terminal_key]
                register = reg_eq.rbus_register(sw_pin, bus)
                _set_bit(bitstream, register, 1, "RBUS {} {}".format(bus, terminal_key), set_sources)
            continue
            ...
            for entry in entries:
                ...
                if has_suffix:
                    ...
                    _set_bit(
                        ...
                        register,
                        value,
                        "SBUS {} {} {}".format(bus, terminal_key, connection),
                        set_sources,
                    )
                    continue
                ...
                _set_bit(
                    ...
                    reg_a,
                    a,
                    "SBUS {}a {} {}".format(bus, terminal_key, connection),
                    set_sources,
                )
                _set_bit(
                    bitstream,
                    reg_b,
                    b,
                    "SBUS {}b {} {}".format(bus, terminal_key, connection),
                    set_sources,
                )
            continue
