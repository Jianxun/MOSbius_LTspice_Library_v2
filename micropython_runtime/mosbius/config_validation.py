...
def _canonical_terminal_name(value):
    if not isinstance(value, str):
        raise ValueError("terminal must be string")
    base = value.split("#", 1)[0].strip()
    if not base:
        raise ValueError("terminal '{}' is empty before '#'".format(value))
    return base


def _canonical_bus_name(value):
    if not isinstance(value, str):
        raise ValueError("bus must be string")
...
def _parse_terminal_and_mode(value):
    ...
    if "@" not in value:
        return _canonical_terminal_name(value), None, False
    terminal, mode = value.rsplit("@", 1)
    terminal = _canonical_terminal_name(terminal)
...
def validate_and_normalize_config(config, pin_to_sw_matrix):
    ...
    for bus_raw, entries in raw_connections.items():
        bus = _canonical_bus_name(bus_raw)
        if not isinstance(entries, list):
            raise ValueError("connections.{} must be a list".format(bus_raw))
        ...
        if bus.startswith("RBUS"):
            ...
            for i, terminal in enumerate(entries):
                ...
                canonical = _canonical_terminal_name(terminal)
                if canonical not in pin_to_sw_matrix:
                    raise ValueError("connections.{}[{}] unknown terminal '{}'".format(bus, i, terminal))
                pin_list.append(canonical)
